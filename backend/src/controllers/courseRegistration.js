...
courseController.AllCourses = function (req, res) {
    if ((req.params.termId || req.query.termId) && (req.params.depId || req.query.depId) ) {
        ...
        let formattedDepIds = depId.split(',').map(i => `'${i}'`).join();
        let sql = `SELECT c.*, it.*
    FROM
        term_course AS tc
            JOIN
    ...
    WHERE
        ...
        db.query(sql, (err, courses) => checkResults(
            req,
            res,
            err,
            courses,
...
courseController.AddCourses = function (req, res) {
        const body = req.body;
        if (!(Object.keys(body).length === 2) || !body.hasOwnProperty('userId') || !body.hasOwnProperty('courseId')) {
...
courseController.DeleteCourses = function (req, res) {
    if ((req.params.courseId || req.query.courseId) && (req.params.uid || req.query.uid) ) {
